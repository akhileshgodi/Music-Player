...
public class LevenshteinsDistance <T>
{
	public int getDistance(T element, T element2) 
	{
        ...
        /*Consider two arrays S and T to be treated as first and second respectively.*/
        ...
        for (i = 1; i <= n; i++)
        {
            for (j = 1; j <= m; j++)
            {
                if (string1.charAt (i - 1) == string2.charAt(j - 1) )
                	...
                	distance[i][j] = findMinimum(distance[i-1][j]+1, distance[i][j-1]+1, distance[i-1][j-1] + 1);
            }
        }
